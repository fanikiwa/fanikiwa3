	...
	if (!resp.code) {
		...
		for (var i = 0; i < resp.result.items.length; i++) {
			if (resp.result.items[i] != null) {
				var totalToPay = resp.result.items[i].loanBalance
				...
				loanTable += '<td>' + resp.result.items[i].interestRate
						+ '</td>';
				loanTable += '<td>'
						+ resp.result.items[i].loanBalance.formatMoney(2)
						+ '</td>';
				loanTable += '<td>'
				...
				loanTable += '<td><a href="#" onclick="Repaid('
						+ resp.result.items[i].id + ')">Repaid</a> </td>';
				loanTable += '<td><a href="#" onclick="WriteOff('
						+ resp.result.items[i].id + ')">Write Off</a> </td>';
...
function Repaid(id) {
	sessionStorage.repaymentscheduleloanid = id;
	window.location.href = "/Views/Loans/LoanRepaid.html";
}
