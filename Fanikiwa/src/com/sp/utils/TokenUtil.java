...
public class TokenUtil {
	public static void SendToken(Userprofile user, String token)
			throws UnsupportedEncodingException {
		SendToken("SMS", user, token);
	}

	public static void SendToken(String sendMethod, Userprofile user,
		...
		if (sendMethod.equals("Web")) {
			...
			sb.append("http://www.kufanikiwa.co.ke/Activate?token=" + token
					+ "&email=" + user.getUserId() + "&method=web");
			String body = sb.toString();

			MailUtil.sendMail(user.getUserId(), "Registration [" + token + "]",
					body);
	...
	public static void SetUserToken(Userprofile user, Date genDate) {
		...
		user.setActivationTokenExpiryDate(DateExtension.addMinutes(genDate,
