...
public class TransactionType {
	...
	/*
	 * When true, the commission amount in the {commissionAmount} field is
	 * absolute and not a percentage
	 */
	...
	/*
	 * When set, if an account used in this transaction type is does not exist,
	 * the value is posted in the suspenseCrAccount or suspenseDrAccount defined
	 * here. The suspenseDrAccount & suspenseCrAccount accounts are not tested
	 * for existence When Not Set, an exception is thrown
	 */
	...
	/*
	 * if true, the computed commission is removed from the transaction itself.
	 * 3 transactions are created instead of 4.
	 */
	private boolean chargeCommissionToTransaction;

	/*
	 * D|C. If D, the account being debited is charged commission otherwise the
	 * credit account is charged
	 */
	private String chargeWho;

	/*
	 * [L|T\F] L = Lookup commission from a table. T = Compute tiered value from
	 * a Tieredtable F = Flate rate
	 */
	private String commComputationMethod;

	/*
	 * Commission amount used in Flatrate method. Can be an absolute or
	 * percentage. Interpretation is determined by {absolute} field
	 */
	...
	/*
	 * A formatter string with placeholders for formatting the contra commission
	 * narrative. The allowable place holders are in the format
	 * {abstractMoneyTransaction.*} where *= abstractMoneyTransaction properties
	 * Defaults to "{ShortCode} Comm{DebitAccount}" -->
	 */
	private String commissionContraNarrative;

	/*
	 * The account where commission is credited. It defaults to
	 * Config["COMMISSIONACCOUNT"] if not set.
	 */
	...
	/*
	 * A formatter string with placeholders for formatting the main commission
	 * narrative. The allowable place holders are in the format
	 * {abstractMoneyTransaction.*} where *= abstractMoneyTransaction properties
	...
	/*
	 * Used to post commission transaction if this transaction canCharge
	 * commission. If not set, defaults to
	...
	/*
	 * [D|C] Determines how to treate the amount for Main and Contra
	 * transactions. When set to 1) D - the amount in Main transaction is -ve
	 * and the amount in Contra transaction is +ve 2) C - opposite of 1 above
	 */
	 ...
	 * The allowable place holders are in the format
	 * {abstractMoneyTransaction.*} where *= abstractMoneyTransaction properties
	 * Defaults to "{ShortCode}" --> that evaluates to the shortcode of the
	 * transaction
	 */
	private String defaultContraNarrative;

	/*
	 * Default main account for the transaction. If set or withdrawal
	 * transaction, it overrides the Config[CASHACCOUNT]
	 */
	...
	/*
	 * [0|1|2]For future use. 0 - Transaction can be used both in
	 * background(System) or Dialog(user) 1 - Dialog only 2 - System only
	 */
	...
	/*
	 * When set, the transaction shall be posted without checking LimitFlag or
	 * PassFlag of the account. i.e can post even to blocked/disabled/overdrawn
	 * accounts.
	 */
	...
	/*
	 * Identifies the table to be used by either Tiered or Lookup computation
	 * methods.
	 */
	...
	/*
	 * [0|1|2] For future use. Used to draw the screen for dialog use. 0 - Draw
	 * a Single Entry View 2 - Draw a Double Entry View 3 - Draw a Multi Entry
	 * View
	 */
	private short txnTypeView;

	/*
	 * The number of days amount remains uncleared. The value should be
	 * positive. Added to the postdate to get valuedate
	 */
