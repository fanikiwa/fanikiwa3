...
public class MpesaIPNTestMessageEndpoint {
	...
	public CollectionResponse<MpesaTestIPNMessage> listMpesaTestIPNMessage(
		...
		Query<MpesaTestIPNMessage> query = ofy().load().type(
				MpesaTestIPNMessage.class);
	 ...
	 * POST method.
	 *
	 * @param mpesaIPNMessage
	 *            the entity to be inserted.
	 * @return The inserted entity.
	 */

	@ApiMethod(name = "insertMpesaTestIPNMessage")
	public MpesaTestIPNMessage insertMpesaTestIPNMessage(
			MpesaTestIPNMessage mpesaIPNMessage) throws ConflictException {
	...
	public MpesaTestIPNMessage updateMpesaTestIPNMessage(
			MpesaTestIPNMessage mpesaIPNMessage) throws NotFoundException {
		if (findRecord(mpesaIPNMessage.getId()) == null) {
	...
	public void removeMpesaTestIPNMessage(@Named("id") Long id)
	...
	public MpesaTestIPNMessage isMpesaTestIPNMessageExists(
			MpesaTestIPNMessage msg) {
	...
	public CollectionResponse<MpesaTestIPNMessage> ListNewMpesaTestIPNMessages(
		...
		Query<MpesaTestIPNMessage> query = ofy().load()
				.type(MpesaTestIPNMessage.class).filter("status", "New");
		return GetMpesaTestIPNMessagesFromQuery(query, cursorString, count);
	}

	private CollectionResponse<MpesaTestIPNMessage> GetMpesaTestIPNMessagesFromQuery(
			Query<MpesaTestIPNMessage> query, String cursorString, Integer count) {
		...
		return CollectionResponse.<MpesaTestIPNMessage> builder()
