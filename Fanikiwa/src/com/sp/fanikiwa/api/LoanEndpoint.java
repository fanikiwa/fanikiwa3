...
public class LoanEndpoint {
	...
	public CollectionResponse<Loan> NextIntAccrualLoanByDate(
		...
		Query<Loan> query = ofy().load().type(Loan.class)
				.filter("nextIntAccrualDate <=", date).filter("status", "Open");
	...
	public CollectionResponse<Loan> NextIntAppLoanByDate(
			@Named("date") Date date) {
		Query<Loan> query = ofy().load().type(Loan.class)
				.filter("nextIntAppDate <=", date).filter("status", "Open");
	...
	// public ArrayList<Installment> GetLoanRepaymentSchedule(Loan loan) {
	//
	...
	// double balance = loan.getAmount();
	// if (term > 0) {
	// instamount = (double) loan.getAmount() / (double) loan.getTerm();
	// }
	//
	...
	@ApiMethod(name = "reverseMemberLoans")
	public RequestResult ReverseMemberLoans(@Named("id") Long memberid) {
		...
		try {
			...
			re.setSuccess(true);
			re.setResultMessage("No Loan Closed");
			return re;
