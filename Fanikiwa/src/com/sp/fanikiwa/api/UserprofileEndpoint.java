...
import com.sp.fanikiwa.entity.ChangePasswordDTO;
import com.sp.fanikiwa.entity.RequestResult;
import com.sp.fanikiwa.entity.ResetPasswordDTO;
...
public class UserprofileEndpoint {
	...
	public RequestResult removeUserprofile(@Named("id") String id) {
		...
		try {
			...
			re.setResultMessage("User [ " + id + " ]  Removed");
			re.setSuccess(true);
			return re;
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public RequestResult createUserprofile(Userprofile userprofile) {
		...
		try {
			...
			try {
				HashedPwd = PasswordHash.createHash(userprofile.getPwd());
				userprofile.setPwd(HashedPwd); // think of encrypting
			...
			TokenUtil.SetUserToken(userprofile, userprofile.getCreateDate());
	...
	@ApiMethod(name = "resetPasswordRequestToken")
	public RequestResult resetPasswordRequestToken(
		...
		try {
			Userprofile user = null;
			user = findRecord(resetPasswordDTO.getEmail());
			if (user == null) {
				re.setSuccess(false);
				re.setResultMessage("User " + resetPasswordDTO.getEmail()
						+ " does not exist.");
				return re;
			}
			// set activation token
			...
			ofy().save().entities(user).now();
			TokenUtil.SendToken(resetPasswordDTO.getActivationMethod(), user,
					user.getToken());
	...
	@ApiMethod(name = "changePasswordRequestToken")
	public RequestResult changePasswordRequestToken(
		...
		try {
			Userprofile user = null;
			user = findRecord(changePasswordDTO.getEmail());
			if (user == null) {
				re.setSuccess(false);
				re.setResultMessage("User [ " + changePasswordDTO.getEmail()
						+ " ] does not exist.");
				return re;
			}
			// set activation token
			...
			ofy().save().entities(user).now();
			TokenUtil.SendToken(changePasswordDTO.getActivationMethod(), user,
					user.getToken());
	...
	public RequestResult changePassword(ChangePasswordDTO changePasswordDTO) {
		RequestResult re = new RequestResult();
		re.setSuccess(false);
		re.setResultMessage("Not Successful");
		try {
			...
			if (!user.getToken().equals(changePasswordDTO.getToken())) {
				re.setSuccess(false);
				re.setResultMessage("User token not valid: Illegal token used ");
				return re;
			}

			user.setPwd(changePasswordDTO.getPwd());
	...
	public RequestResult resetPassword(ResetPasswordDTO resetPasswordDTO) {
		RequestResult re = new RequestResult();
		re.setSuccess(false);
		re.setResultMessage("Not Successful");
		try {
			...
			if (!user.getToken().equals(resetPasswordDTO.getToken())) {
				re.setSuccess(false);
				re.setResultMessage("User token not valid: Illegal token used ");
				return re;
			}
			user.setPwd(resetPasswordDTO.getPwd());
	...
	public RequestResult Login(@Named("userId") String userID,
		...
		if (authenticated && activated) {
			...
			re.setResultMessage("created date:"
					+ DateExtension.FormatDate(user.getCreateDate()));
			re.setClientToken(user);
			return re;
