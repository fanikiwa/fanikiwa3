...
public class LendingGroupEndpoint {
	...
	private static final Logger log = Logger
			.getLogger(LendingGroupEndpoint.class.getName());
		...
		if (Lendinggroup.getGroupName() != null) {
			if (findRecord(Lendinggroup.getGroupName()) != null) {
				throw new ConflictException("Group "
						+ Lendinggroup.getGroupName() + " already exists");
			}
		}
		ofy().save().entities(Lendinggroup).now();
		return Lendinggroup;
				...
				new Work<Lendinggroup>() {
					public Lendinggroup run() {
						...
						try {
							...
							re.setSuccess(true);
							re.setResultMessage("Group "
									+ lendingGroupDTO.getGroupName()
									+ " Created.");
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
	...
	@ApiMethod(name = "pulgeMemberLendinggroup")
	public RequestResult PulgeMemberLendinggroup(@Named("id") Long memberid) {
		...
		try {
			...
			Collection<Lendinggroup> ldgps = retrieveLendinggroupsByCreator(
					member.getEmail(), null, null).getItems();
			for (Lendinggroup lg : ldgps) {
				removeLendinggroup(lg.getGroupName());
			}
			re.setSuccess(true);
			return re;
		} catch (Exception e) {
