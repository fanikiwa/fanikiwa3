...
public class OfferReceipientEndpoint {
	...
	public RequestResult removeOfferReceipient(@Named("id") Long id) {
		RequestResult re = new RequestResult();
		re.setSuccess(false);
		re.setResultMessage("Not Successful");
		try {
			...
			re.setSuccess(true);
			re.setResultMessage("OfferReceipient[ " + id + " ]  deleted.");
			return re;
		} catch (Exception e) {
			e.printStackTrace();
			re.setSuccess(false);
	...
	public RequestResult PulgeOfferandReceipient(@Named("id") Long memberid) {
		RequestResult re = new RequestResult();
		re.setSuccess(false);
		re.setResultMessage("Not Successful");

		try {
			String msg = "Zero Offers removed";
			...
			Collection<Offer> memberoffers = listOfferReceipient(memberid,
					null, null).getItems();
			for (Offer of : memberoffers) {

				Collection<OfferReceipient> offerreceipients = getOfferReceipients(
						of.getId(), null, null).getItems();
				for (OfferReceipient ofr : offerreceipients) {
					removeOfferReceipient(ofr.getOfferReceipientId());
					msg += "OfferReceipient [ " + ofr.getMember().getEmail()
							+ " ] Removed.<br/>";
				}
				oep.removeOffer(of.getId());
				msg += "Offer [ " + of.getId() + " ] Removed.<br/>";
			}
			re.setSuccess(true);
			re.setResultMessage(msg);
			return re;
		} catch (Exception e) {
			e.printStackTrace();
			re.setSuccess(false);
