...
public class ContactEndpoint {
	...
	public RequestResult insertContact(Contact contact) {
		...
		try {
			...
			re.setSuccess(true);
			re.setResultMessage("Contact Created.<br/>Id = "
	...
	@ApiMethod(name = "retrieveContact")
	public RequestResult retrieveContact(@Named("id") Long id) {
		RequestResult re = new RequestResult();
		re.setSuccess(false);
		re.setResultMessage("Not Successful");
		try {
			Contact contact = findRecord(id);
			if (contact == null) {
				throw new NotFoundException("Contact [ " + id
						+ " ]  does not exist");
			}
			re.setSuccess(true);
			re.setClientToken(contact);
			return re;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
