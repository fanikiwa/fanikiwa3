...
public class MpesaProcessServelet extends HttpServlet {
	private static final Logger log = Logger
			.getLogger(MpesaProcessServelet.class.getName());
	...
	private void ProcessDBMessagesNow(HttpServletRequest request,
		...
		try {
			...
			log.info("STEP 3: Processing " + mpesaMsgs.size()
					+ " new messages...");
			for (MpesaIPNMessage mpesaMsg : mpesaMsgs) {

				log.info("STEP 3.1: Processing mgsid["
						+ mpesaMsg.getMpesa_code() + "] ...");
				MpesaComponent mcomp = new MpesaComponent();
				String str = mcomp.ProcessMessage(mpesaMsg);
				log.info("STEP 3.2: Processed mgsid["
						+ mpesaMsg.getMpesa_code() + "] ...\nResult =" + str);
				...
				log.info("STEP 3.3: DB Status of message id["
						+ mpesaMsg.getMpesa_code() + "] changed to Processed");
