...
public class RegistrationComponent {
	...
	public boolean Authenticated(String telephone, String pwd) {
		...
		Userprofile user = uep.LoginByPhone(telephone, pwd); // this already
																// does proper
																// authentication
		if (user == null) {
			return false;
	...
	public boolean Authorized(String telephone, String pwd) {
		...
		if (m == null)
			return false;
		if (m.getStatus().toUpperCase().equals("A"))
			authorized = true; // member is active, so authorized
	...
	public RequestResult Register(UserDTO m) throws ConflictException,
			NotFoundException {
		MemberEndpoint mep = new MemberEndpoint();
		return mep.Register(m);
	}

	public RequestResult Activate(ActivationDTO acDTO)
