...
public class DiaryComponent {
	...
	static {
		DiaryComponent.register("01VALUEDATED", new JobClearValueDatedItems());
		DiaryComponent.register("02INTACR", new JobAccrueLoanInterest());
		DiaryComponent.register("03INTAPP", new JobApplyLoanInterest());
		DiaryComponent.register("04STO", new JobProcessSTO());
	}

	boolean EnableLog = true;

	private static final Logger log = Logger.getLogger(DiaryComponent.class
			.getName());
	...
	public void RunDiary(Date date) {
		...
		try {
			...
			Collection<Diaryprogramcontrol> controlRec = dDac
					.listDiaryprogramcontrol(null, null).getItems();
			...
			for (Date i = dp.getLastRun(); i.before(date); i = DateExtension
					.addDays(i, 1)) {
				if (EnableLog)
					log.info("Processing started for [" + i + "]");
