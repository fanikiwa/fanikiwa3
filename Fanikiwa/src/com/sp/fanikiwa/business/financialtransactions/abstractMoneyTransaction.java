...
public abstract class abstractMoneyTransaction
{
	...
	public abstractMoneyTransaction(TransactionType ttype,
        String shortCode, Date postDate,
        Account drAccount,
        Account crAccount,
        double amount,
        boolean forcePost,
    ...
    {
        if (ttype == null) throw new NullPointerException("Transaction type cannot be null");
    ...
    private void Initialize()
    {
    	...
    	if(TransactionType.getChargeWho() != null && TransactionType.getChargeWho().toUpperCase().equals("C"))
        {
        ...
        if (TransactionType.getCommissionCrAccount() != null && TransactionType.getCommissionCrAccount() != 0) 
        	CommissionCreditAccount =  GLUtil.GetAccount(TransactionType.getCommissionCrAccount());
        //Use generic comm txn type
        CommissionTransactionType = Config.GetTransactionType("COMMISSIONTRANSACTIONTYPE");
        if (TransactionType.getCommissionTransactionType() != null && TransactionType.getCommissionTransactionType() != 0) 
        	CommissionTransactionType = GLUtil.GetTransactionType(TransactionType.getCommissionTransactionType());

        //value dates
        if (TransactionType.getValueDateOffset()> 0) valuedate =DateExtension.addDays(PostDate,TransactionType.getValueDateOffset());
    ...
    {
    	 Transaction dr = GetTransaction(getDebitTransaction(), fmt.getDrNarrativeFommatter(), format);
    	 Transaction cr = GetTransaction(getCreditTransaction(), fmt.getCrNarrativeFommatter(), format);
    	 DoubleEntry de = new DoubleEntry();
    	 de.setDr( dr); de.setCr(cr);
         return de;
    }
    public  Transaction GetTransaction(Transaction txn, String Fommatter, boolean format)
    {
        if(format)  txn.setNarrative(StringExtension.format(Fommatter, this));
        return txn;
    }
    ...
    public double EvaluateCommission()
    {
        ...
        return cc.ComputeCommissionByTransactionType(this.Amount, this.TransactionType);
    }
    public DoubleEntry GetCommissionTransaction(NarrativeFormat fmt)
    { return GetCommissionTransaction(fmt, false); }
    public DoubleEntry GetCommissionTransaction(NarrativeFormat fmt, boolean format)
    {
    	Transaction dr = GetTransaction(getDebitCommissionTransaction(), fmt.getDrNarrativeCommissionFommatter(), format);
   	 Transaction cr = GetTransaction(getCreditCommissionTransaction(), fmt.getCrNarrativeCommissionFommatter(), format);
   	 DoubleEntry de = new DoubleEntry();
   	de.setDr( dr); de.setCr(cr);
        return de;
    }

    public  List<Transaction> GetChargeCommissionToTransaction(NarrativeFormat fmt)
    { return GetChargeCommissionToTransaction(fmt, false); }
    public  List<Transaction> GetChargeCommissionToTransaction(NarrativeFormat fmt,boolean format)
    {
        ...
        txns.add(GetTransaction(dr, fmt.getDrNarrativeCommissionFommatter(),format));
        txns.add(GetTransaction(cr, fmt.getDrNarrativeCommissionFommatter(),format));
        txns.add(GetTransaction(cmm, fmt.getDrNarrativeCommissionFommatter(),format));
        return txns;
    }
    ...
    public  List<Transaction> GetTransactionsIncludingCommission(NarrativeFormat MainFmt, NarrativeFormat CommFmt)
    {
        ...
        if (ChargeCommission)
        {
            ...
            if(cmmDe.getDr() != null) txns.add(cmmDe.getDr());
            if (cmmDe.getCr() != null) txns.add(cmmDe.getCr());
        }
        return txns;
    }
