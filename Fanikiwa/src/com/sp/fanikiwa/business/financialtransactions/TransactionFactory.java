...
public class TransactionFactory {
	public static List<Transaction> CreateTransactionsFromTransactionType(
			TransactionType tt, Date PostDate, Account DrAccount,
			Account CrAccount, double Amount, String Narr, String reference,
			String UserID, String Authorizer) throws Exception {
		/*
		...
		if (tt.getCanSuspend()) {
			...
			if (tt.getSuspenseDrAccount() != null) {
				...
				if (suspDr == 0)
					throw new BadRequestException(
							"SuspenseDrAccount does not exist");
			} else {
				throw new BadRequestException(
						"SuspenseDrAccount does not exist");
			}
			...
			if (tt.getSuspenseCrAccount() != null) {
				...
				if (suspCr == 0)
					throw new BadRequestException(
							"SuspenseCrAccount does not exist");
			} else {
				throw new BadRequestException(
						"SuspenseCrAccount does not exist");
			}
			...
			if (!GLUtil.AccountExists(MainAcc.getAccountID()))
				MainAcc = aep.getAccount(suspDr);
			if (!GLUtil.AccountExists(ContraAcc.getAccountID()))
				ContraAcc = aep.getAccount(suspCr);
		}

		// 3. Get the transaction type

		GenericTransaction gt = new GenericTransaction(tt, tt.getShortCode(),
				PostDate, MainAcc, ContraAcc, Amount, tt.getForcePost(),
				tt.getStatFlag(), UserID, Authorizer, reference);

		// 4. Configure gt according to transaction type
		// eg a) dp.CommissionTransactionType = MPESACOMMACC
		if (tt.getChargeCommission()
				&& tt.getCommissionTransactionType() != null) {
			gt.CommissionTransactionType = GLUtil.GetTransactionType(tt
					.getCommissionTransactionType());
		} else // use defualt
		{
			gt.CommissionTransactionType = Config
	...
	public static List<Transaction> Deposit(TransactionType tt,
			Account MainAccount, Account ContraAccount, double Amount,
			String Narr, String reference, String UserID, String Authorizer)
			throws Exception {

		return CreateTransactionsFromTransactionType(tt, new Date(),
				MainAccount, ContraAccount, Amount, Narr, reference, UserID,
				Authorizer);
	}

	public static List<Transaction> MpesaDeposit(Long AccountId, double Amount,
		...
		TransactionType tt = Config
				.GetTransactionType("MPESADEPOSITTRANSACTIONTYPE");
		...
		return Deposit(tt, Draccount, ContraAcc, Amount, Narr, reference,
				"SYS", "SYS");
	}

	public static List<Transaction> Withdraw(Long AccountId, double Amount,
			String Narr, String reference, String AccountSymbol,
		...
		return WithdrawImpl(tt, MainAcc, ContraAcc, Amount, Narr, reference,
				"SYS", "SYS");
	}

	public static List<Transaction> WithdrawImpl(
			TransactionType TransactionType, Account MainAccount,
			Account ContraAccount, double Amount, String Narr,
			String reference, String UserID, String Authorizer)
			throws Exception {
		return CreateTransactionsFromTransactionType(TransactionType,
				new Date(), MainAccount, ContraAccount, Amount, Narr,
				reference, UserID, Authorizer);
	}

	// Diary Transactions

	public static List<Transaction> LoanRepayment(
			TransactionType TransactionType, Date PostDate,
			Account MainAccount, Account ContraAccount, double Amount,
			String Narr, String reference, String UserID, String Authorizer)
			throws Exception {
		return CreateTransactionsFromTransactionType(TransactionType, PostDate,
				MainAccount, ContraAccount, Amount, Narr, reference, UserID,
				Authorizer);
	}
}
