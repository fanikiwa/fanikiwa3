...
public class LoanComponent {
	 ...
	 */
	public RequestResult PrepayLoan(Loan loan, double amount, Date date) throws Exception {
		return PayLoan( loan, amount ,  date);
	}
	
	public RequestResult PayLoan(Loan loan, double amount, Date date) throws Exception {
		...
		if(((newloan.getAmount()- newloan.getAmountPrincipalPaid() )<=0 ) &&
			(newloan.getAccruedInterest()+newloan.getAccruedIntInSusp()) <= 0	)
		{
	...
	public RequestResult ApplyAccruedInterest(Loan loan, Date date)
		...
		if (re.isSuccess()) {  //try applying interest in the current accounts
			...
			if (res.isSuccess()) {
				...
				//update statistics
				loan.setNoOfInterestPaymentsMade(loan.getNoOfInterestPaymentsMade()+1);
	...
	public void AccrueInterest(Loan loan, Date date) throws NotFoundException {
		...
		if(loan.getAmountPrincipalDefaulted()>0)  penaltyInterest =this.computeInterest("Suspended",loan,date);
		...
		loan.setAccruedInterestToDate(loan.getAccruedInterestToDate()+normalInterest);
	...
	private RequestResult ReduceLoanObligation(Loan loan, Account loanAccount,
		...
		GenericTransaction ltxn = new GenericTransaction(lpay, "LEP", new Date(),
				investmentAccount, loanAccount,amount, false, true, AUTHORIZER, USERID, loan
						.getId().toString());
	...
	private RequestResult PostPrincipal(Loan loan,Account DrAmount, Account CrAccount,
		...
		List<Transaction> txns = TransactionFactory.LoanRepayment(tt,
				new Date(), DrAmount, CrAccount, Amount, "Loan repayment", loan.getId().toString(),
				USERID, AUTHORIZER);
		RequestResult re = GLUtil.Simulate(txns);
		if (re.isSuccess()) {
			return TransactionPost.Post(txns);
	...
	private List<Transaction> LoanInterestApplicationTransactions(Loan loan) {
		...
		Account paidAccount = GLUtil.GetAccount(loan.getIntPaidAccount()); //lender.CurrentAcc or loan account
		...
		GenericTransaction inttxn = new GenericTransaction(intt, "INT",
				...
				loan.getAccruedInterest() + loan.getAccruedIntInSusp(), false, true, AUTHORIZER, USERID, loan
						.getId().toString());
