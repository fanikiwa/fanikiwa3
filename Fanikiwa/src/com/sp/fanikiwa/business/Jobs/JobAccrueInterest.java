...
public class JobAccrueInterest implements IJobItem {
	...
	private static final Logger log = Logger.getLogger(JobAccrueInterest.class
			.getName());
	...
	private void AccrueInterest(Date d) {
		...
		Collection<Account> _Schedule = ep.NextIntAccrualAccountsByDate(d)
				.getItems();

		if (EnableLog)
			log.info("Processing [" + _Schedule.size() + "] transactions");
		for (Account acc : _Schedule) {
			...
			try {
				...
				if (EnableLog)
					log.info("Processing Accrual [" + acc.getAccountID()
							+ "] completed");
			} catch (Exception ex) {
				log.log(Level.SEVERE,
						"An error occurred while processing Accrual["
								+ acc.getAccountID() + "] completed\n", ex);
			}
	...
	private void ProcessAccrual(Account acc, Date date) {
		...
		if (!StringExtension.isNullOrEmpty(method)
			...
			intr = ic.ComputeCompoundInterest(acc.getBookBalance(),
					GetAccountTerm(acc, date),
					(double) GetEffectiveIntRate(acc));
		} else {
	...
	private int GetAccountTerm(Account acc, Date date) // return term in days
	{
		 ...
		 * 
		 * switch(term.toUpperCase()) //D1, D360, D365, M1, M30, Y { case "D1":
		 * return 1; case "D360": return 360; case "D365": return 365; case "M":
		 * return DateExtension.DaysOfTheMonth(date); case "M30": return 30;
		 * case "Y": return DateExtension.DaysOfTheYear(date); default: return
		 * 1; }
		 */
	}
