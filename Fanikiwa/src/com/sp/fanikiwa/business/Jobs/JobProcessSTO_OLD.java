...
public class JobProcessSTO_OLD implements IJobItem {
	...
	private static final Logger log = Logger.getLogger(JobProcessSTO_OLD.class
			.getName());
	...
	private void RunSTOs(Date date) {
		...
		Collection<STO> _ScheduleTransactions = st.nextSTOByDate(date)
				.getItems();
		...
		for (STO _sto : _ScheduleTransactions) {
			...
			try {
				String msg = String.format(
						"Processing STO Id[{0}], NextPayDate[{7}] "
								+ "DrAccount[{1}],  " + "CrAccount[{2}],  "
								+ "PayAmount[{3}],  " + "TotalToPay[{4}],  "
								+ "CrTxnType[{5}],  " + "DrTxnType[{6}] ",
						_sto.getId(), _sto.getDrAccount(), _sto.getCrAccount(),
						_sto.getPayAmount(), _sto.getTotalToPay(),
						_sto.getCrTxnType(), _sto.getDrTxnType(),
						_sto.getNextPayDate());
				if (EnableLog)
					log.info(msg);
				...
				if (EnableLog)
					log.info("Processing STO[" + _sto.getId() + "] completed");
			} catch (Exception ex) {
				log.log(Level.SEVERE, "An error occurred while processing STO["
						+ _sto.getId() + "] completed\n", ex);
			}
	...
	private void ProcessNormalSTO(STO _sto, Date date) throws Exception {
		...
		if (_sto.getAmountPaid() >= _sto.getTotalToPay()) {
			// log
			if (EnableLog)
				log.info("STO " + _sto.getId() + "already cleared...");
			return;
		}
		...
		/*
		 * Compute commission taking care of flags 1 _sto.ChargeCommFlag 2
		 * _sto.CommissionPaidFlag 3 _stoCommSourceFlag
		 */
		commission = comm.GetCommissionAmountForSTO(_sto);

		AmountNeeded = commission + AmountToPay; // commission will be 0 if not
													// charged
		...
		/*
		 * Scenario A - STOpartial pay = false Scenario A1 - Account has enough
		 * money for both repayment and commission Scenario A2 - Account has
		 * enough money for commission only Scenario A3 - Account does not have
		 * enough money even for commission
		 * 
		 * Scenario B - STOpartial pay = true Scenario B1 - Account has enough
		 * money for both repayment and commission Scenario B2 - Account has
		 * enough money for commission only Scenario B3 - Account does not have
		 * enough money even for commission
		 */

		// First and foremost charge commission
		/*
		 * 1. ChargeCommission takes care of commission == 0
		 * sto.CommissionPaidFlag sto.ChargeCommFlag
		 * sto.STOCommFreqFlag.NoCommission
		 */
		ChargeCommission(_sto.getDrAccount(), _sto.getCrAccount(), _sto,
				commission, date);
		...
		if (AmountAvailable >= AmountNeeded) {
			...
			AmountToPay = _sto.getPayAmount();
			ProcessInstallment(_sto.getDrAccount(), _sto.getCrAccount(), _sto,
					AmountToPay, date, AmountAvailable);
			return;
		} else {
			...
			if (_sto.getPartialPay()) {
				...
				ProcessInstallment(_sto.getDrAccount(), _sto.getCrAccount(),
						_sto, AmountToPay, date, AmountToPay);
				return;
			}

			// Scenario A3 && Scenario B3
			if (AmountAvailable < commission) {
				ProcessNonPay(_sto.getDrAccount(), _sto.getCrAccount(), _sto,
						commission, date, AmountAvailable);
				return;
			}
		}
	...
	private void ProcessSweepSTO(STO _sto, Date date) throws Exception {
		/*
		 * Sweep everything in the account. Does not consider the value
		 * sto.Payamount field
		 */
		AccountEndpoint aep = new AccountEndpoint();
		Account acc = aep.getAccount(_sto.getDrAccount());
		...
		CommissionComponent comm = new CommissionComponent();
		double commission = comm.GetCommissionAmountForSTO(_sto); // get the
																	// commission
																	// for
																	// running
																	// the sto
																	...
																	// transaction
																	// type
		double PayAmount = AmountAvailable - commission; // minus comm coz it is
															// going to b
															// charged herebelow
															// separately
		ChargeCommission(_sto.getDrAccount(), _sto.getCrAccount(), _sto,
				commission, date);
		ProcessInstallment(_sto.getDrAccount(), _sto.getCrAccount(), _sto,
				PayAmount, date, PayAmount);
	}

	private void ProcessInstallment(Long DrAcc, Long CrAcc, STO _sto,
			double PayAmount, Date date, double AmountAvailable)
			throws Exception {
		...
		_sto.setAmountDefaulted(_sto.getAmountDefaulted()
				+ (_sto.getPayAmount() - _sto.getPayAmount()));
		if (_sto.getAmountDefaulted() > 0) {
			_sto.setNoOfDefaults(_sto.getNoOfDefaults() + 1);
			_sto.setNextPayDate(DateExtension.addDays(date, 1)); // keep
																	// checking
																	// every day
		}
		...
		// 3. log
		if (EnableLog)
			log.info("Payment NOT made.: AmountNeeded[" + _sto.getPayAmount()
					+ "], AmountAvailable[" + AmountAvailable + "]");
	}

	private void ProcessNonPay(Long Borrower, Long Investor, STO _sto,
			double commission, Date date, double AmountAvailable)
			throws NotFoundException {
		_sto.setNextPayDate(DateExtension.addDays(date, 1)); // keep checking
																// every day
		_sto.setNoOfDefaults(_sto.getNoOfDefaults() + 1);
		...
		if (EnableLog)
			log.info("Payment NOT made.: AmountNeeded[" + _sto.getPayAmount()
					+ "],  Commission[" + commission + "] AmountAvailable["
					+ AmountAvailable + "]");
	}

	private boolean PostSTOTransactions(Long DrAcc, Long CrAcc, STO sto,
			double Amount, Date stoCreatedDate) throws Exception {
		TransactionTypeEndpoint tep = new TransactionTypeEndpoint();
		List<Transaction> txns = TransactionFactory.LoanRepayment(
				tep.getTransactionType(sto.getDrTxnType()), stoCreatedDate,
				GLUtil.GetAccount(DrAcc), GLUtil.GetAccount(CrAcc), Amount,
				"Repayment", "", "Diary", "Diary");
		// now request posting service to post
		TransactionPost.Post(txns);
		return true;
	}

	private void ChargeCommission(Long DrAcc, Long CrAcc, STO sto,
			double commission, Date date) throws NotFoundException {
		if (commission == 0)
			return;

		if (sto.getCommissionPaidFlag())
			return;

		if (!sto.getChargeCommFlag())
			return;

		if (STOCommFreqFlag.values()[sto.getCommFreqFlag()] == STOCommFreqFlag.NoCommission)
			return;

		ChargeCommissionImpl(DrAcc, CrAcc, sto, commission, date);
	}

	private void ChargeCommissionImpl(Long DrAcc, Long CrAcc, STO sto,
			double commission, Date date) throws NotFoundException {
		...
		/*
		 * 1. If r.ChargeWho == 'L' // Borrower to pay commission 2. If
		 * r.ChargeWho == 'I' // Investor to pay commission 3. If r.ChargeWho ==
		 * 'B' // Both to pay commission
		 */
		switch (STOCommissionChargeWho.values()[sto.getChargeWho()]) {
		case Borrower:
			txns = MakeCommissionTxns(forcePost, contraref, DrAcc, sto,
					commission, date);
			break;
		case Investor:
			txns = MakeCommissionTxns(forcePost, contraref, CrAcc, sto,
					commission, date);
			break;
		case BothBorrowerAndInvestor:
			txns = MakeCommissionTxns(forcePost, contraref, DrAcc, sto,
					commission, date);
			txns.addAll(MakeCommissionTxns(forcePost, contraref, CrAcc, sto,
					commission, date));
			break;
		default:
			break;
		...
		// Post
		// now request posting service to post with limit checking according to
		// sto set up
		TransactionPost.BatchPost(txns,
				PostingCheckFlag.values()[sto.getLimitFlag()]);

		// Mark STO Commission as paid
		/*
		 * Mark comm as paid if CommFreqFlag==
		 * STOCommFreqFlag.PayCommissionPerOnce
		 */
	...
	private List<Transaction> MakeCommissionTxns(boolean forcePost,
			String contraref, Long DrAcc, STO sto, double Amount, Date date) {
		List<Transaction> txns = new ArrayList<Transaction>();
		TransactionType tt = GLUtil.GetTransactionType(sto.getDrTxnType());

		if (Amount != 0) {
			GenericTransaction ltxn = new GenericTransaction(tt, "LES",
					new Date(), GLUtil.GetAccount(DrAcc), GLUtil.GetAccount(sto
							.getCommissionAccount()), Amount, forcePost, true,
					"SYS", "SYS", "STO Comm");
	...
	// private void ClearInvestment(Member Borrower, Member Investor, int TTYpe,
	// double Amount, Date stoCreatedDate)
	// {
	// try
	// {
	// //build commission transactions
	...
	// MakeClearInvestmentTxns(contraref, Borrower, Investor, TTYpe, Amount,
	// txns, stoCreatedDate);
	// //Post
	...
	// {
	// infoLog.Info(string.Format("Posted ClearInvestment Transaction. AccountID: {0}, Amount: {1}, PostDate: {2}, RecordDate: {3}, Narrative: {4} ",
	// txn.AccountID, txn.Amount, txn.PostDate, txn.RecordDate, txn.Narrative));
	// }
	...
	// public void MakeClearInvestmentTxns(string contraref, Member Borrower,
	// Member Investor, int TxnType, double Amount, List<Transaction> txns, Date
	// stoCreateDate)
	// {
	// try
	// {
	...
	// //investor investmentaccount was credited when loan was created, now
	// debit to show repayment
	...
	// //borrower loanaccount was debited when loan was created, now credit to
	// show the repayment
