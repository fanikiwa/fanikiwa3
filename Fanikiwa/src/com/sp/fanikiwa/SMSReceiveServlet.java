	...
	private void ProcessSMS(HttpServletRequest request,
		...
		try {
			...
			if (msg != null) {
				...
				if (Enablelog)
					log.info("Creating fanikiwa message from sms["
				...
				if (Enablelog)
					log.info("Processing fanikiwa message["
							+ fMsg.getFanikiwaMessageType().name() + "] ");
				...
				if (Enablelog)
					log.info("Sending response  result[" + result + "] ");
